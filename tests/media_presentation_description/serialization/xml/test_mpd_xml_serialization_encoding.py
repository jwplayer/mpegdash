...
class MPDXMLSerializationEncodingTest(unittest.TestCase):
    ...
    def test_default_xml_encoding(self):
        mpd = MediaPresentationDescription(profile='urn:mpeg:dash:profile:full:2011', minimum_buffer_time=1)
        xml = etree.fromstring(str(mpd.to_xml()))
        assert xml.getroottree().docinfo.encoding == 'UTF-8'

    def test_default_utf8_encoding(self):
        mpd = MediaPresentationDescription(profile='urn:mpeg:dash:profile:full:2011', minimum_buffer_time=1)
        xml = etree.fromstring(str(mpd.to_xml(encoding='UTF-8')))
        assert xml.getroottree().docinfo.encoding == 'UTF-8'

    def test_default_ascii_encoding(self):
        mpd = MediaPresentationDescription(profile='urn:mpeg:dash:profile:full:2011', minimum_buffer_time=1)
        xml = etree.fromstring(str(mpd.to_xml(encoding='ASCII')))
